...
// Duration is now hardcoded to 30 seconds
...
export default function Home() {
  ...
  // Duration is hardcoded to 30 seconds
        ...
        for (const line of lines) {
          try {
            ...
            if (event.type === "log") {
              ...
              const progressMatch = event.data.match(/PROGRESS:\s*(\d+)%\s*-\s*(.+)/);
              if (progressMatch) {
                const percent = parseInt(progressMatch[1]);
                const stepDesc = progressMatch[2];
                setFacelessProgress(percent);
                setFacelessCurrentStep(stepDesc);
              }
          ...
          {activeTab === "story" ? (
            ...
            <div className="max-w-[1240px] mx-auto px-6 py-4 space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="space-y-5">
                ...
                {/* Top Section - URL Input (Full Width) */}
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                      {MUSIC_MOODS.map((m) => (
                        <button
                          ...
                          disabled={isProcessingFaceless}
                          className={`p-4 rounded-lg border transition-all ${facelessMusic === m.id ? "border-purple-500/40 bg-purple-500/10" : "border-white/5 bg-black/20 hover:border-purple-500/20"}`}
                        >
                ...
                {/* Language - Full Width (Duration removed) */}
                <div className="bg-white/[0.02] border border-white/5 rounded-xl p-5">
                  <label className="block text-base font-semibold text-white/50 mb-4">Language (30 sec videos)</label>
